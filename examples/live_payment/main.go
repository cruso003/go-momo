...
func main() {
	...
	config, err := momomtn.NewConfig(
		momomtn.Production,
		momomtn.WithSubscriptionKey(os.Getenv("MOMO_PROD_SUBSCRIPTION_KEY")),
		momomtn.WithDisbursementKey(os.Getenv("MOMO_PROD_DISBURSEMENT_KEY")),
		momomtn.WithCallbackHost(os.Getenv("MOMO_PROD_CALLBACK_HOST")),
		momomtn.WithHost(os.Getenv("MOMO_PROD_HOST")),
		momomtn.WithTargetEnvironment(os.Getenv("MOMO_PROD_TARGET_ENVIRONMENT")),
		momomtn.WithAPIUser(os.Getenv("MOMO_PROD_API_USER")),
		momomtn.WithAPIKey(os.Getenv("MOMO_PROD_API_KEY")),
		momomtn.WithCurrency(os.Getenv("MOMO_PROD_CURRENCY")),
	)
	if err != nil {
	...
	for i := 0; i < maxPolls; i++ {
		...
		if status.Status == momomtn.Successful {
			...
			break
		} else if status.Status == momomtn.Failed || status.Status == momomtn.Rejected {
	...
	if paymentSuccessful {
		...
		if response == "y" || response == "Y" {
			...
			disbursementIdempotencyKey := momomtn.GenerateIdempotencyKey(
				"live_disburse",
				phone,
			...
			for i := 0; i < maxPolls; i++ {
				...
				if status.Status == momomtn.Successful {
					...
					break
				} else if status.Status == momomtn.Failed ||
